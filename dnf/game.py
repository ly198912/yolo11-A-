...
class Game:
    ...
    def __init__(
        ...
        special_skill_ready: bool | None = None,
    ):
        self._obj = obj or []
        self._width = width
        self._height = height
    ...
    def _pick_up(self, obj_box: Sequence[float]) -> None:
        ...
        elif abs_dx <= x_deadzone or abs_dx <= y_sweep_x_range:
    ...
    def _pickup_can_interrupt_route(self, obj_box: Sequence[float]) -> bool:
        ...
        if parts & {"LEFT", "RIGHT"} and abs_dy > cross_limit:
            return False
        return not (parts & {"UP", "DOWN"} and abs_dx > cross_limit)
    ...
    def _route_search_progress_reached(
        ...
        if "DOWN" in command and current_y - start_y >= y_amplitude:
            return True
        return bool("UP" in command and start_y - current_y >= y_amplitude)
    ...
    def _has_usable_route_door(self) -> bool:
        ...
        if self._get_route_forward_door(usable_doors, direction_hint):
            return True
        return bool(self._would_push_outside(direction_hint) and usable_doors)
